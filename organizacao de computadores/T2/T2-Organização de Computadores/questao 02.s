...
# Repita o problema anterior, escrevendo um programa para a divis„o de n˙mero de 32 bits, em
# ponto inteiro, com sinal.
...
###############################################################################################################		
	...
	# carrega o inteiro lido em $t2
		la  $t2, 0($a0)		  
	#verifica se o numero DIVIsor È negativo
		la $t3, 0
		slt $a0, $t2, $t3
		la $t9, ($a0)
		mul $t9, $t9, -1
...
s_corrige:
	la $t5, ($s0)
	# verificaÁ„o sinais do resto e do quociente
	#sinal do resto segue o sinal do dividendo
        	bne $t8, -1, if3
		mul $t5, $t5, -1
	if3:
	# se divisor e dividendo tem sinais opostos
	# muda sinal quociente
		beq $t9, $t8, if4
		mul $t4, $t4, -1
	if4: 
        # imprime valores
...
imprimir: # $a0 string # $a1 valor para syscall 56
	...
	la $a1, ($t5) # carregar o endereÁo da string a ser impressa em $ a1
	syscall # call sistema operacional para executar a operaÁ„o de impress„o
	li $v0, 56 # carregue o cÛdigo adequado da chamada do sistema para registrar $ v0;  
...
string0:	.asciiz "ESTE … PROGRAMA DE DIVIS√O DE NUMEROS EM PONTO INTEIRO COM SINAL\nENTRE -2^31 a 2^31 - 1\nDESEJA CONTINUAR?" # mensagem inicial do programa
